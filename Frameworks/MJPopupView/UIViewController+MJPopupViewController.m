...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType dismissed:(void(^)(void))dismissed
{
        UIView *sourceView = [self topView];
        UIView *popupView = [sourceView viewWithTag:kMJPopupViewTag];
        UIView *overlayView = [sourceView viewWithTag:kMJOverlayViewTag];
...
- (void)fadeViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    ...
    // Set starting properties
    popupView.frame = popupEndRect;
    if([self respondsToSelector:@selector(stickPopupToBottom)]){
        popupView.center = CGPointMake(overlayView.bounds.size.width / 2, overlayView.bounds.size.height - popupView.bounds.size.height / 2.0);
    }
    popupView.alpha = 0.0f;
